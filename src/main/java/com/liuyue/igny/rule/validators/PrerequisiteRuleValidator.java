...
public class PrerequisiteRuleValidator {
    ...
    @SafeVarargs
    public static <V, T> ValueValidator<V> createValidator(RuleAccessor<T> rule, T... needValues) {
        Set<T> allowedValues = new HashSet<>(Arrays.asList(needValues));
        return new ValueValidator<>() {
            @Override
            public boolean validate(V newValue) {
    ...
    @SafeVarargs
    public static <V, T> ValueValidator<V> createValidatorWithBreak(RuleAccessor<T> rule, T... breakValues) {
