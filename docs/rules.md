...
## 始终可用管理员方块 (alwaysAvailableGameMasterBlocks)
...
## 重新引入末影龙删除跳过 (enderDragonRemoveSkipReintroduce) `MC>=26.1`
...
允许将物品放入工作台的结果槽并将其分解。  
同一产物有多个配方时，对着结果槽按丢弃键可轮换到下一个配方。  
该规则同样适用于合成器。将合成器命名为配方索引对应的数字即可使用（例如对应第一个配方时命名为1）。
